...
REVIEW_SCHEMA = {
    ...
    "HelpfulnessNumerator": pl.Int32,
    "HelpfulnessDenominator": pl.Int32,
    "Score": pl.Float32,
    "Time": pl.Int32,
    "Summary": pl.String,
    "Text": pl.String,
...
def cleaned_reviews_lazy(csv_path: Path = RAW_DATA_PATH) -> pl.LazyFrame:
    return (
        ...
        .with_columns(pl.struct(DUPLICATE_SUBSET).hash().alias("_dupe_hash"))
        .unique(subset=["_dupe_hash"], keep="first")
        .drop("_dupe_hash")
        .with_columns(
            [
                pl.col("Id").cast(pl.Int32),
                pl.col("HelpfulnessNumerator").cast(pl.Int32),
                pl.col("HelpfulnessDenominator").cast(pl.Int32),
...
def build_quality_report(
    ...
    quality_checks = {
        "missing_product_id": cleaned_reviews.filter(pl.col("ProductId") == "").height,
        "missing_summary_and_text": cleaned_reviews.filter(
            (pl.col("Summary") == "") & (pl.col("Text") == "")
        ).height,
        "score_out_of_range": cleaned_reviews.filter(
            ~pl.col("Score").is_between(1, 5, closed="both")
        ).height,
        "helpfulness_invalid": cleaned_reviews.filter(
            pl.col("HelpfulnessNumerator") > pl.col("HelpfulnessDenominator")
        ).height,
    }
...
def build_product_documents(cleaned_reviews: pl.DataFrame) -> pl.DataFrame:
    ...
    aggregated = ranked.group_by("ProductId").agg(
        [
            pl.len().cast(pl.Int32).alias("review_count"),
...
def write_pipeline_outputs(
    ...
    cleaned_reviews_csv_path = processed_dir / "reviews_cleaned.csv"
    ...
    try:
        ...
        cleaned_reviews_path = None
        print(f"Warning: failed to write cleaned parquet: {exc}")
    cleaned_reviews.write_csv(cleaned_reviews_csv_path)
    ...
    return {
        ...
        "cleaned_reviews_csv": cleaned_reviews_csv_path,
